...
# -*- coding: utf-8 -*-
"""
Created on Thu Aug 22 11:19:19 2019
...
def display_images( img, nr=5, nc=4, s1=28, s2=28):
    ...
    new_img=np.reshape(img_np, (3*(nr*nc),s1,s2))
    disp=np.zeros((3*nr*s1,3*nc*s2))
    ...
    for i in range(nr):
        for j in range(nc):
                ...
                r0=3*i*s1
                c0=3*j*s2
                disp[r0:(r0+3*s1),c0:(c0+3*s2)]=new_img[read_data,:,:]
...
#        loss = criterion(yhat.to(device),which_digit[0][0].to(device))
        recon_batch, mu, logvar = model(data)
        loss = loss_function(recon_batch, data, mu, logvar, beta)
#        loss = nn.MSELoss()
        loss.backward()
